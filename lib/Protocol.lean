...
namespace ConcreteRB
  ...
  @[export send_message]
  def send_message (p: ConcreteRBProtocol) (node_state: ConcreteRBState) (round: ConcreteRound) : ConcreteRBState ├ù Array ConcreteRBPacket :=
    let (new_state, packet_list) := p.procInternal node_state round
    (new_state, List.toArray packet_list)

  @[export handle_message]
  def handle_message (p: ConcreteRBProtocol) (node_state: ConcreteRBState) (src: ConcreteAddress) (msg: ConcreteRBMessage) : ConcreteRBState ├ù Array ConcreteRBPacket :=

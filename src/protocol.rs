...
// TODO: might want to parameterize over Proof type too. idk
...
impl<V: Clone> ProvableBroadcast<V> {
    ...
    fn proc_msg(&self, st: &mut NodeState<V>, src: Address, msg: Message<V>) -> Vec<Packet<V>> {
        // TODO: there's some other `routine_check` thing being done in the actual implementation.
        // investigate what that's for.
        let NodeState {
            address,
            sent,
            counter,
                        ...
                        true => {
                            ...
                            match partial_signatures.contains(&(src, partial_signature)) {
                                false => {
                                    ...
                                    if has_enough_signatures && !has_combined_signature {
